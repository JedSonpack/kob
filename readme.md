...
```text
Web(/agent-lab/) -> backend(任务 API + AgentWorkflowService) -> OpenAI-compatible LLM
...
依赖：JDK 1.8（`/Library/Java/JavaVirtualMachines/jdk-1.8.jdk`）、Maven、Node 20、MySQL 8.0。运行前用 `node --version` 确认当前运行时为 v20；使用 nvm 时可执行 `nvm use 20`。`scripts/dev.sh` 默认导出 `KOB_AGENT_LLM_PROVIDER=fake`，自动化与本地演示不调真实模型。
...
| 验证项 | 状态 | 证据或限制 |
| --- | --- | --- |
| 后端全量 Maven 测试 | 已通过 | 161 项测试，Reactor 5/5 成功 |
| 前端 test / lint / build | 已通过 | Node 20 下 37 项测试通过，lint 与 build 退出码为 0 |
| 开发脚本行为 | 受当前沙箱限制 | 本轮端口探测权限不足；任务 5 的提升权限验收曾通过 1/1 |
| 现有对战 Playwright | 已通过 | 任务 5 验收覆盖 3/3 场景 |
| Agent Lab Playwright | 待最新网络复验 | 修复前完整 Fake LLM 闭环已通过；最新安全修复轻量回归 6/6，通过后因沙箱网络限制未完成全栈 GREEN |

## 真实实验结果

真实模型实验记录存放于 [`docs/agent-lab/experiments/`](docs/agent-lab/experiments/)，命名 `YYYYMMDD-HHmm-模型名.md`。结论判定（至少 2/3 同方向改善才称「可重复提升」）与表格列见实验协议。

当前状态：尚未配置真实模型，也未完成 3 次真实实验，因此本节不填写胜率或提升结论。
