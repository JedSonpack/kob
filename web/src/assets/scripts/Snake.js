...
export class Snake extends AcGameObject {
    ...
    check_tail_increasing() {  // 检测当前回合，蛇的长度是否增加 要变长，蛇尾不动即可，否则到倒数第二个位置
    ...
    next_step() {  // 将蛇的状态变为走下一步
        ...
        this.eye_direction = d; //蛇头方向跟移动一样
    ...
    update_move() {
        ...
        if (distance < this.eps) {  // 走到目标点了
            ...
            if (!this.check_tail_increasing()) {  // 蛇不变长，此时倒数第一个和倒数第二个就重合了
                this.cells.pop(); //去掉蛇尾
            }
        } else {
            ...
            if (!this.check_tail_increasing()) { //移动的时候，蛇尾不增加，蛇尾跟着走
                const k = this.cells.length;
